...
public class MjolnirManager {
    ...
    /** Launch Mjölnir from the player's hand */
    public void throwHammer(Player player, ItemStack hammer) {
        ...
        // Spawn a snowball as the visual projectile (invisible via particle rendering)
        Snowball projectile = player.launchProjectile(Snowball.class);
        projectile.setVelocity(player.getEyeLocation().getDirection().multiply(2.0));
        projectile.setGravity(false);
        projectile.setCustomName("mjolnir_projectile");
    ...
    /** Called every tick — handles outward flight, reversal, and return */
    private void tick() {
        ...
        while (it.hasNext()) {
            ...
            // Hammer trail particles
            player.getWorld().spawnParticle(Particle.ELECTRIC_SPARK, projLoc, 3, 0.1, 0.1, 0.1, 0.02);

            // Phase 1: outward (0-600ms), Phase 2: return (600ms+)
            if (elapsed < 600) {
                // Keep flying outward — check for entity hits
                ...
                if (projLoc.distanceSquared(player.getLocation()) < 2.5) {
                    ...
                    player.sendActionBar(net.kyori.adventure.text.Component.text(
                        "⚡ Mjölnir returned!", net.kyori.adventure.text.format.NamedTextColor.GOLD));
                }
            }
        }
    }
    ...
    private void hitNearbyEntities(ThrowData data, Player player, Location loc) {
        for (Entity entity : loc.getWorld().getNearbyEntities(loc, 1.5, 1.5, 1.5)) {
            ...
            if (data.hitEntities.contains(entity.getUniqueId())) continue; // don't double hit

            // Deal damage
            living.damage(8.0, player);
            data.hitEntities.add(entity.getUniqueId());

            // Strike lightning visually (no damage lightning)
            loc.getWorld().strikeLightningEffect(loc);
            loc.getWorld().playSound(loc, Sound.ENTITY_LIGHTNING_BOLT_IMPACT, 0.8f, 1.0f);

            // Rain bonus: extra lightning damage
